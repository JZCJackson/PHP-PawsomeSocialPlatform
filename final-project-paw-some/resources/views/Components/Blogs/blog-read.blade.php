                ...
                <div class="lg:w-3/4 space-y-5">
                    ...
                    <div class="card">
                        ...
                        <div class="h-44 mb-4 md:h-72 overflow-hidden relative rounded-t-lg w-full">
                            <img src="/storage/blogs/{{$b->picture}}" alt=""
                                 class="w-full h-full absolute inset-0 object-cover">
                        </div>
                        <div class="p-7">

                            <h1 class="lg:text-3xl text-2xl font-semibold mb-6"> {{$b->title}} </h1>

                            <div class="flex items-center space-x-3 my-4 pb-4 border-b border-gray-100">
                                <img src="/storage/profile/{{\App\Models\User::where('id',$b->author_id)->value('picture') }}" alt=""
                                     class="w-10 rounded-full object-cover">
                                <div>
                                    <div class="text-base font-semibold"> {{ \App\Models\User::where('id',$b->author_id)->value('name')  }} </div>
                                    <div class="text-xs"> Published on {{\Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $b->created_at)->formatLocalized('%B %d, %Y')}}</div>
                                </div>
                            </div>
